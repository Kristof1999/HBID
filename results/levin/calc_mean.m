...
mean_im_one_kernel = zeros(8, 2);
mean_kernel_all_im = zeros(1, 8);
for k = 1:8
    mean_kssim = zeros(1,4);
    mean_xssim = zeros(4,2);
    for x = 1:4
        ...
        % tx = imread("blur/im"+x+"_kernel"+k+"_img.png");
        % tx = imread("selfdeblur/im"+x+"_kernel"+k+"_img_x.png");
        ...
        tx = imread(name+"/im"+x+"_kernel"+k+"_img_x.png");
        x2 = imread("gt/im"+x+".png");
        tx = im2double(tx);
        x2 = im2double(x2);
        [p, s, e] = comp_upto_shift(tx, x2, maxshift);
    ...
    mean_im_one_kernel(k, :) = mean(mean_xssim, 1);
end
...
% legend(["SelfDeblur", "Saját"])
% title("Kernelek szerinti átlagos kép SSIM értékek")
